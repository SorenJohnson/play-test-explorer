...
def parse_card_values(path: Path) -> dict[str, float]:
    """Parse CardValues.csv into a {card_name: learned_value} dict.

    Generated by the `evaluate-cards` CLI command via OLS regression.
    ...
    with open(path, newline="") as f:
        ...
        for row in reader:
            name = row["Card"].strip()
            raw = (row.get("Learned_Value") or "0").strip()
